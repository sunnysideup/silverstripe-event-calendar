    ...
    function applyMonthJson (month, year) {
      ...
      for (date in json) {
        if (json[date].events.length) {
          $('[data-date="' + date + '"]')
            .addClass('hasEvent')
            .attr('title', json[date].events.join('\n'))
        }
      }
    }
      ...
      if ($e.data('start') && $e.data('end')) {
        ...
        for (loopTime = startTime; loopTime < endTime; loopTime += 86400000) {
          ...
          var stamp =
            loopDay.getFullYear() +
            '-' +
            calendar.pad(loopDay.getMonth() + 1) +
            '-' +
            calendar.pad(loopDay.getDate())
    ...
    $('.calendar-widget').each(function () {
      var opts = {
        ...
        onInit: function (calendar) {
          ...
          loadMonthJson(this_month, this_year)

          loadMonthJson(prev_month, prev_year)

          loadMonthJson(next_month, next_year)
